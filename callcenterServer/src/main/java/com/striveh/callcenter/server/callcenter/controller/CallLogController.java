...
/**
 * @功能:【callLog 呼叫日志表】controller @项目名:callcenterServer @作者:xxx @日期:2020-04-06 12:13:47 @说明：
 *
 * <pre></pre>
 */
@RestController
...
public class CallLogController extends BaseController<CallLogPojo> {
  ...
  /**
   * 获取项目呼叫结果 1 pageNum Integer √ 第几页 2 pageSize Integer √ 每页条数 3 projectCode String X 项目代码 4
   * callTask Code String X 任务代码
   */
  ...
  public ResponseData getList(CallLogPojo callLog) {
    ...
    try {
      ...
      if (!StringTool.isEmpty(callLog.getResultDetail())
          && callLog.getResultDetail().contains(",")) {
  ...
  public ResponseData getCallListForRepeat(CallLogPojo callLog) {
    ...
    try {
      ...
      if (baseRedisDaoDBCache.exists(
          ERedisCacheKey.CALLCENTER_TASK_CALLLIST_REPEAT.getCode() + callLog.getCallTaskCode())) {
        ...
        if (callTaskPojo.getCallType().equals(2)) {
          ...
          if (callLog.getResultDetail().contains("50")
              && callLog.getResultDetail().contains("55")) {
  ...
  private void extracted(CallLogPojo callLog, Map<String, Object> data) {
    List<String> items =
        baseRedisDaoDBCache.lrange(
            ERedisCacheKey.CALLCENTER_TASK_CALLLIST_REPEAT.getCode() + callLog.getCallTaskCode(),
            (callLog.getPageNum() - 1) * callLog.getPageSize(),
            callLog.getPageNum() * callLog.getPageSize() - 1);
