...
/**
 * @功能:【callProject 呼叫项目表】controller @项目名:callcenterServer @作者:xxx @日期:2020-04-06 12:13:48 @说明：
 *
 * <pre></pre>
 */
@RestController
...
public class CallProjectController extends BaseController<CallProjectPojo> {
  ...
  /** 创建项目获得分机 1: projectCode String √ 项目代码 2: seats Integer X 坐席数 */
      ...
      if (StringUtils.isEmpty(projectCode)) {
        ...
        if (callProjectPojo == null) {
          ...
          for (FreeswitchPojo e : freeswitchPojos) {
            ...
            List<CallProjectPojo> callProjectPojos =
                this.callProjectService.selectList(callProjectPojo1);
  ...
  /**
   * 坐席认领项目分机 1:certificate String √ 坐席领取分机凭证 2:extension String X 分配给坐席的分机号 3:type Integer √
   * 2坐席顾问3项目经理
   */
  ...
  /** 给项目分配坐席 1:projectCode String √ 项目代码 2:username String √ 分配给坐席的分机号 */
    ...
    try {
      if (StringUtils.isEmpty(projectCode)) {
        ...
        if (callProjectPojo == null) {
          ...
          FreeswitchPojo freeswitchPojo =
              this.freeswitchService.getCacheDataByKey(
                  new FreeswitchPojo(callProjectPojo.getFreeswitchId()));
          userinfoPojo.setServers(freeswitchPojo.getWebSipPhone());
          userinfoPojo.setNativeServers(freeswitchPojo.getNativeSipPhone());
          userinfoPojo.setUri(
              "sip:" + userinfoPojo.getUsername() + "@" + freeswitchPojo.getNativeSipPhone());
        }
      }
    } catch (Exception ex) {
  ...
  /** 获取项目分机状态 1 projectCode String √ 项目代码 */
  ...
  /** 置忙置闲 1 workStatus String √ 0不可通话、1可通话 2 username String √ 分配给坐席的分机号 */
        ...
        if (workStatus != null) {
          if (userinfoPojo != null && userinfoPojo.getWorkStatus() > -1) {
            ...
            if (callTaskPojo != null
                && !callTaskPojo.getScheduleType().equals(4)
                && (callTaskPojo.getStatus().equals(2) || callTaskPojo.getStatus().equals(3))) {
            ...
            } else if (StringUtils.isNotBlank(userinfoPojo.getCalluuid())
                && workStatus == 0
              ...
              logger.info(
                ...
                && workStatus == 0
                && userinfoPojo.getSessionStatus().equals(10)) {
              baseRedisDao.saveOrUpdate(
                  ERedisCacheKey.CALLCENTER_AGENT_WORK_STATUS_SESSION_AFTER.getCode()
                      + userinfoPojo.getUsername(),
                  -1,
                  600);
            }
            ...
            msgOperations.convertAndSend(
                "/topic/status/" + userinfoPojo.getUsername(), userinfoPojo);
            msgOperations.convertAndSend(
                "/topic/callExtStatus/" + callProjectPojo.getProjectCode(), userinfoPojo);
  ...
  /** 置忙置闲 1 workStatus String √ 0不可通话、1可通话 2 projectCode String √ 项目代码 */
      ...
      if (StringUtils.isEmpty(projectCode)) {
        ...
        if (callProjectPojo == null) {
          ...
          List<UserinfoPojo> userinfoList =
              this.userInfoServiceApi.getListByProjectId(callProjectPojo.getId());
  ...
  /** 分机禁用启用 1 workStatus String √ -1禁用0启用 2 username String √ 分配给坐席的分机号 */
          ...
          if (workStatus == -1) {
            ...
            if (callTaskPojo != null
              ...
              logger.info(
                ...
                && userinfoPojo.getSessionStatus().equals(10)) {
              baseRedisDao.saveOrUpdate(
                  ERedisCacheKey.CALLCENTER_AGENT_WORK_STATUS_SESSION_AFTER.getCode() + username,
                  -1,
                  600);
  ...
  /** 完结项目 1 projectCode String √ 项目代码 */
  ...
  /** 上传语音文件 1 qiniuStoreKey String √ 语音文件在七牛文件存储服务器里的存储key 2 voiceCode String √ 语音代码 */
  ...
  public ResponseData sendToUser(String projectCode, String username, String content) {
    ...
    try {
      msgOperations.convertAndSend(
          "/topic/notify"
              + (StringUtils.isNotEmpty(projectCode) ? "/" + projectCode : "")
  ...
  public void idle(String jsonStr) {
    ...
    if (userinfoPojo == null
        || userinfoPojo.getSessionStatus().equals(10)
        || !userinfoPojo.getWorkStatus().equals(0)
        || userinfoPojo.getStatus().equals(0)) {
      logger.info("坐席{}置闲,不行", map.get("username"));
      return;
    } else if (StringUtils.isNotEmpty(
        baseRedisDao.get(
            ERedisCacheKey.CALLCENTER_AGENT_WORK_STATUS_IDLE.getCode() + userinfoPojo.getUsername(),
            String.class))) {
      notify.put("data", "置闲中，请等待结果");
      notify.put("msgType", "success");
      msgOperations.convertAndSend(
          "/topic/notify/" + map.get("projectCode") + "/" + map.get("username"),
      ...
      if (userinfoPojo.getWorkStatus() > -1) {
        try {
          ...
          logger.info(
              ...
              callTaskPojo.getCallTaskCode(),
              userinfoPojo.getUsername(),
              EslHelper.formatEslMessage(eslMe));
  ...
  public void stopEavesdrop(String jsonStr) {
    ...
    if (projectCode == null) {
      EslMessage eslMessage =
          inboundClient.sendSyncApiCommand(
              inboundClient.option().serverOptions().get(0).addr(),
              "uuid_kill",
              listener.getCalluuid());
    } else {
      CallProjectPojo callProjectPojo = new CallProjectPojo();
      callProjectPojo.setProjectCode(projectCode);
      callProjectPojo = this.callProjectService.selectUnique(callProjectPojo);
  ...
  public void conferenceByUsername(String jsonStr) {
    ...
    if (eslMessage.getBodyLines().get(0).contains("not found")) {
      inboundClient.sendAsyncApiCommand(
          freeswitchService.getServerAddr(callProjectPojo.getFreeswitchId()),
          "conference",
          projectCode + " bgdial user/" + listenerExt);
      logger.info("项目经理{}发起会议{}", listenerExt, projectCode);
    }
    inboundClient.sendAsyncApiCommand(
        freeswitchService.getServerAddr(callProjectPojo.getFreeswitchId()),
        "conference",
        projectCode + " bgdial user/" + username);
  ...
  public void playVoice(String jsonStr) {
    ...
    // 给坐席播放
    EslMessage uuid_displaceForAgent =
        inboundClient.sendSyncApiCommand(
            freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
            "uuid_displace",
            userinfoPojo.getCalluuid() + " start " + voicePojo.getPatch() + " 0 mux");
    logger.info(
        "坐席{}uuid_displaceForAgent:{}结果{}",
        username,
        userinfoPojo.getCalluuid(),
        EslHelper.formatEslMessage(uuid_displaceForAgent));
    if (!uuid_displaceForAgent.getBodyLines().get(0).contains("ERR")) {
      // 给接听者播放
      EslMessage uuid_displaceForCall =
          inboundClient.sendSyncApiCommand(
              freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
              "uuid_displace",
              callId + " start " + voicePojo.getPatch() + " 0 mux");
      logger.info(
          "坐席{}uuid_displaceForCall:{}结果{}",
          username,
          callId,
          EslHelper.formatEslMessage(uuid_displaceForCall));
      // 把坐席静音
      EslMessage uuid_audioForAgent =
          inboundClient.sendSyncApiCommand(
              freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
              "uuid_audio",
              userinfoPojo.getCalluuid() + " start read mute -4");
      logger.info(
          "坐席{}uuid_audioForAgent:{}结果{}",
          username,
          userinfoPojo.getCalluuid(),
          EslHelper.formatEslMessage(uuid_audioForAgent));
    }
  }
  ...
  public void stopPlayVoice(String jsonStr) {
    ...
    //		// 把坐席静音恢复
    //		EslMessage uuid_audioForAgent =
    // inboundClient.sendSyncApiCommand(freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
    //				"uuid_audio",userinfoPojo.getCalluuid()+" stop");
    //		logger.info("坐席{}uuid_audioForAgent:{}结果{}",username,userinfoPojo.getCalluuid(),
    // EslHelper.formatEslMessage(uuid_audioForAgent));
    ...
    EslMessage uuid_displaceForAgent =
        inboundClient.sendSyncApiCommand(
            freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
            "uuid_displace",
            userinfoPojo.getCalluuid() + " stop " + voicePojo.getPatch() + " 0 mux");
    logger.info(
        "坐席{}uuid_displaceForAgent:{}结果{}",
        username,
        userinfoPojo.getCalluuid(),
    ...
    EslMessage uuid_displaceForCall =
        inboundClient.sendSyncApiCommand(
            freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
            "uuid_displace",
            callId + " stop " + voicePojo.getPatch() + " 0 mux");
    logger.info(
        "坐席{}uuid_displaceForCall:{}结果{}",
        username,
        callId,
  ...
  public void transfer(String jsonStr) {
    ...
    notify.put("type", "message");
    notify.put("duration", "3000");
    if (otherUserinfo.getStatus().equals(0)
        || otherUserinfo.getSessionStatus().equals(10)
        || otherUserinfo.getWorkStatus().equals(1)) {
      notify.put("data", otherUsername + "状态不行，无法转移");
      notify.put("msgType", "error");
    } else {
      EslMessage uuid_transfer =
          inboundClient.sendSyncApiCommand(
              freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
              "uuid_transfer",
              userinfoPojo.getCalluuid()
                  + " -bleg 'bridge:{call_transfer=true}user/"
                  + otherUsername
                  + "' inline");
      logger.info(
          "坐席{}uuid_transfer:{}结果{}", username, callId, EslHelper.formatEslMessage(uuid_transfer));
