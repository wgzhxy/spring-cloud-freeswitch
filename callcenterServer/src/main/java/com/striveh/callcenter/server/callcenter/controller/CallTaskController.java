...
public class CallTaskController extends BaseController<CallTaskPojo> {
  ...
  /** 呼叫任务启动 1 callTaskCode String √ 任务代码 */
  ...
  /**
   * 设置任务并发数 1 callTaskCode String √ 任务代码 2 callGWs JSONObject √
   * 线路列表[{"code":"1001","concurrent":100},{"code":"1002","concurrent":200}] 3 rate Integer √ 拨打倍率
   */
  ...
  /** 给坐席分配号码（预览式拨打） 1 callTaskCode String √ 任务代码 2 count Integer √ 分配数量 */
  ...
  /** 呼叫任务暂停 1 callTaskCode String √ 任务代码 */
  ...
  /** 呼叫任务结束 1 callTaskCode String √ 任务代码 */
  ...
  /** 呼叫任务追加 1 callTaskCode String √ 任务代码 */
  ...
  /** 获取任务进度 1 callTaskCode String √ 任务代码 */
  ...
  public void pmCallAgent(String jsonStr) {
    ...
    try {
      ...
      if (gatewayPojo.getRegisterType() == 1) {
        ...
        EslMessage eslMessage =
            inboundClient.sendSyncApiCommand(
                serverAddr, "sofia_contact", gatewayPojo.getUsername());
        logger.info(
      ...
      EslMessage eslMessage = inboundClient.sendSyncApiCommand(serverAddr, "create_uuid", null);
      ...
      logger.info(
          ...
          username,
          callStr,
          inboundClient.sendAsyncApiCommand(serverAddr, "originate", callStr));
  ...
  public void pmByeAgent(String jsonStr) {
    ...
    if (projectCode == null) {
      EslMessage eslMessage =
          inboundClient.sendSyncApiCommand(
              inboundClient.option().serverOptions().get(0).addr(), "uuid_kill", map.get("callId"));
      logger.info(
          "项目经理给坐席打电话挂断{}：{},结果{}",
          map.get("username"),
          map.get("callId"),
    ...
    if (userinfoPojo.getProjectId().equals(7145L)) {
      ...
      if (userinfoPojo.getSessionStatus().equals(0)) {
        ...
        msgOperations.convertAndSend(
            "/topic/bye/" + userinfoPojo.getUsername(), JsonTool.getJsonString(body));
      ...
      if (userinfoPojo.getSessionStatus().equals(10)) {
        ...
        logger.info(
            ...
            map.get("username"),
            map.get("callId"),
            EslHelper.formatEslMessage(eslMessagePark));
        if (eslMessagePark.getBodyLines().get(0).contains("ERR")) {
          ...
          String eslMessage =
              inboundClient.sendAsyncApiCommand(
                  freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
                  "uuid_broadcast",
                  map.get("callId") + " " + playbackPath + "evaluation.mp3 aleg");
          logger.info(
              "坐席{}挂断 {},uuid_broadcast结果{}", map.get("username"), map.get("callId"), eslMessage);
  ...
  public void task(String jsonStr) {
    ...
    if (callTaskPojo != null) {
      msgOperations.convertAndSend(
          "/topic/task/" + map.get("projectCode") + "/" + map.get("username"), callTaskPojo);
    }
  }
    ...
    inboundClient
        ...
        .forEach(
            e -> {
              String eslMessage =
                  inboundClient.sendAsyncApiCommand(
                      e.addr(), "sofia profile external rescan reloadxml", null);
      ...
      if (StringUtils.isNotEmpty(gatewayPojo.getUsername())) {
        ...
        gatewayFileWriter.append(
            "        <param name=\"password\" value=\""
                + gatewayPojo.getPassword()
                + "\"/>"
                + "\n");
      }
      gatewayFileWriter.append(
          "        <param name=\"realm\" value=\"" + gatewayPojo.getRealm() + "\"/>" + "\n");
      gatewayFileWriter.append(
          "        <param name=\"register\" value=\"" + gatewayPojo.getRegister() + "\"/>" + "\n");
