...
public class UnregisterEventHandler implements ISubEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    ...
    UserinfoPojo userinfoPojo =
        this.userInfoServiceApi.getByUsername(event.getEventHeaders().get("from-user"));
    if (userinfoPojo != null && userinfoPojo.getType().equals(2)) {
      ...
      if (StringUtils.isNotBlank(userinfoPojo.getCalluuid())
          ...
          && (DateTool.getTimestamp().getTime() - userinfoPojo.getLastSessionBeginTime().getTime())
              > 1000 * 60 * 3) {
        EslMessage eslMessage =
            inboundClient.sendSyncApiCommand(addr, "uuid_kill", userinfoPojo.getCalluuid());
        log.info(
            "摘机坐席{}注销通话中置忙{}", userinfoPojo.getUsername(), EslHelper.formatEslMessage(eslMessage));
        userinfoPojo.setWorkStatus(0);
      } else if (StringUtils.isNotBlank(userinfoPojo.getCalluuid())
          && userinfoPojo.getWorkStatus().equals(1)
      ...
      if (callTask != null
          ...
          && (callTask.getStatus().equals(2) || callTask.getStatus().equals(3))) {
        String param =
