...
public class ChannelDestroyEslEventHandler implements EslEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    log.info(
        "ChannelDestroyEslEventHandler handle addr[{}] EslEvent[{}].",
        addr,
          ...
          if (userinfoPojo != null
            ...
            if (StringUtils.isNotEmpty(
              ...
              msgOperations.convertAndSend(
                  "/topic/notify/" + userinfoPojo.getUsername(),
              ...
              log.info(
                  "坐席{}置忙，call_uuid:{}",
                  userinfoPojo.getUsername(),
                  event.getEventHeaders().get("variable_call_uuid"));
            }
          }
        } else if (StringUtils.isNotEmpty(
            event.getEventHeaders().get("variable_sip_h_X-Call-Task-Code"))) {
          UserinfoPojo userinfoPojo =
              this.userInfoServiceApi.getByUsername(
                  event.getEventHeaders().get("variable_dialed_user"));
          // 11660呼入项目拒接处理
