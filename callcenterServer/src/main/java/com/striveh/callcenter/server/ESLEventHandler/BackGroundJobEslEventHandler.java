...
public class BackGroundJobEslEventHandler implements EslEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    log.info(
        "BackGroundJobEslEventHandler handle addr[{}] EslEvent[{}].",
        addr,
    ...
    try {
      ...
      if (event.getEventBodyLines().get(0).contains("GATEWAY_DOWN")
        ...
        if (event.getEventHeaders().get("Job-Command-Arg").contains("answer,fifo:")) {
          ...
          String username =
              callStr.substring(callStr.lastIndexOf("/") + 1, callStr.lastIndexOf("answer") - 1);
          baseRedisDao.delete(
              ERedisCacheKey.CALLCENTER_AGENT_WORK_STATUS_IDLE.getCode() + username);
          ...
          msgOperations.convertAndSend(
              "/topic/notify/" + username, JsonTool.getFormatJsonString(notify));
          return;
        }

        EslMessage message = inboundClient.sendSyncApiCommand(addr, "status", null);
        log.info(
            "DESTINATION_OUT_OF_ORDER FreeSWITCH:{}status:{}",
            addr,
            EslHelper.formatEslMessage(message));

        String callStr = event.getEventHeaders().get("Job-Command-Arg");
        CallLogPojo callLogPojo = new CallLogPojo();
        callLogPojo.setCallTaskCode(
            callStr.substring(callStr.lastIndexOf("=") + 1, callStr.indexOf("}")));
        String telNo = null;
        ...
        if (callLogPojo != null) {
          ...
          baseRedisDao.saveOrUpdate(
              ERedisCacheKey.CALLCENTER_TASK_PAUSE.getCode() + callLogPojo.getCallTaskCode(),
              10,
              10);
