...
/**
 * @功能:【callProject 呼叫项目表】Service @项目名:callcenterServer @作者:xxx @日期:2020-04-06 12:13:48 @说明：
 *
 * <pre></pre>
 */
@Service
public class CallProjectService extends BaseService<CallProjectPojo, CallProjectDao>
    implements ICallProjectService {
  ...
  private synchronized void configFifo() {
    ...
    try {
      ...
      for (CallProjectPojo e : callProjectPojos) {
        fileWriter.append(
            "    <fifo name=\"" + e.getProjectCode() + "\" importance=\"0\"></fifo>" + "\n");
      }
      ...
      inboundClient
          ...
          .forEach(
              e -> {
                EslMessage eslMessageReloadxml =
                    inboundClient.sendSyncApiCommand(e.addr(), "reloadxml", null);
                EslMessage eslMessage =
                    inboundClient.sendSyncApiCommand(e.addr(), "fifo reparse", null);
    ...
    if (projectCode == null) {
      EslMessage eslMessage =
          inboundClient.sendSyncApiCommand(
              inboundClient.option().serverOptions().get(0).addr(), "create_uuid", null);
  ...
  public void conference(String projectCode, String listenerExt) {
    ...
    if (eslMessage.getBodyLines().get(0).contains("not found")) {
      ...
      List<UserinfoPojo> agents =
          this.userInfoServiceApi.getListByProjectIdAndStatus(callProjectPojo.getId(), 1);
          ...
          e -> {
            if (e.getSessionStatus().equals(0) && e.getWorkStatus().equals(0)) {
              inboundClient.sendAsyncApiCommand(
                  ...
                  "conference",
                  projectCode + " bgdial user/" + e.getUsername());
