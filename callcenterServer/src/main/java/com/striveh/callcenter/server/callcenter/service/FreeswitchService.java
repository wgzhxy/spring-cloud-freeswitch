 ...
 */
@Service
public class FreeswitchService extends BaseCacheService<FreeswitchPojo, FreeswitchDao>
  ...
  protected void init() {
    Long lastVer =
        baseRedisDao.get(ERedisCacheKey.KEY_SYS_FREESWITCH_NEW_VER.getCode(), Long.class);
    if (lastVer == null) {
      lastVer = System.currentTimeMillis();
      baseRedisDao.saveOrUpdate(
          ERedisCacheKey.KEY_SYS_FREESWITCH_NEW_VER.getCode(), lastVer); // 更新新版本为当前
    }
    if (cacheLastVer == null || cacheLastVer < lastVer) {
          ...
          .forEach(
              freeswitch -> {
                inboundClient
                    .option()
                    .addServerOption(
                        new ServerOption(freeswitch.getHost(), freeswitch.getPort())
                            .password(freeswitch.getPassword()));
      ...
      inboundClient
          ...
          .addEvents(
              ...
              "BACKGROUND_JOB",
              "DTMF",
              EventNames.PLAYBACK_START,
              EventNames.PLAYBACK_STOP,
              EventNames.MEDIA_BUG_STOP,
              "CUSTOM fifo::info",
