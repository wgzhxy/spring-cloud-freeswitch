  ...
  public void handle(String addr, EslEvent event) {
    ...
    switch (event.getEventHeaders().get("Action")) {
      case "conference-create":
        log.info(
            "ConferenceEventHandler conference-create:{},{}",
            addr,
        ...
        //                agents.forEach(e->{
        //                    inboundClient.sendAsyncApiCommand(addr,"conference",projectCode+"
        // bgdial user/"+e.getUsername());
      ...
      case "add-member":
        ...
        msgOperations.convertAndSend(
            "/topic/callExtStatus/" + event.getEventHeaders().get("Conference-Name"), userinfoPojo);
        ...
        log.info(
            ...
            event.getEventHeaders().get("Conference-Name"));
      ...
      case "del-member":
        ...
        if (userinfoPojo.getType().equals(3)) {
          inboundClient.sendAsyncApiCommand(
              addr, "conference", event.getEventHeaders().get("Conference-Name") + " hup all");
          log.info(
              "项目经理{}离开离开会议,会议{}",
              userinfoPojo.getUsername(),
              event.getEventHeaders().get("Conference-Name"));
