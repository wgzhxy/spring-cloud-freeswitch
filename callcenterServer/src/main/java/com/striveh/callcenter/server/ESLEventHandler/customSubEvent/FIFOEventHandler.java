...
public class FIFOEventHandler implements ISubEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    try {
      ...
      if (event.getEventHeaders().get("FIFO-Action").equals("bridge-caller-start")
          || event.getEventHeaders().get("FIFO-Action").equals("bridge-caller-stop")) {
      ...
      switch (event.getEventHeaders().get("FIFO-Action")) {
        ...
        case "consumer_start":
          log.info(
              "fifoinfoEventHandler consumer_start:{},{}", addr, EslHelper.formatEslEvent(event));
          if (StringUtils.isEmpty(event.getEventHeaders().get("variable_sip_h_X-Call-Task-Code"))) {
            userinfoPojo =
                this.userInfoServiceApi.getByUsername(
                    event.getEventHeaders().get("variable_dialed_user"));
            if (StringUtils.isNotBlank(userinfoPojo.getCalluuid())) {
              EslMessage eslMessage =
                  inboundClient.sendSyncApiCommand(addr, "uuid_kill", userinfoPojo.getCalluuid());
              log.info(
                  "摘机坐席{}重复置闲{},杀掉上一个{}",
            ...
            baseRedisDao.delete(
                ERedisCacheKey.CALLCENTER_AGENT_WORK_STATUS_IDLE.getCode()
                    + userinfoPojo.getUsername());
            ...
            msgOperations.convertAndSend(
                "/topic/notify/" + userinfoPojo.getUsername(), JsonTool.getFormatJsonString(msg));
        ...
        case "bridge-caller-start":
          log.info(
              "fifoinfoEventHandler bridge-caller-start:{},{}",
              addr,
              EslHelper.formatEslEvent(event));
          msgOperations.convertAndSend(
              "/topic/invite/" + event.getEventHeaders().get("Other-Leg-Caller-ID-Number"),
              JsonTool.getJsonString(body));
          userinfoPojo =
              this.userInfoServiceApi.getByUsername(
                  event.getEventHeaders().get("Other-Leg-Caller-ID-Number"));
          userinfoPojo.setSessionStatus(10);
          userinfoPojo.setWorkStatus(1);
          userinfoPojo.setLastSessionBeginTime(DateTool.getTimestamp());

          baseRedisDao.unorderedzrem(
              ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode() + body.get("callTaskCode"),
              body.get("telNo"));
          baseRedisDao.unorderedzadd(
              ERedisCacheKey.CALLCENTER_TASK_SESSION_COUNT.getCode() + body.get("callTaskCode"),
              body.get("telNo"),
              30 * 60);
          ...
          log.info(
              "坐席{}，{}通话开始{}",
              userinfoPojo.getUsername(),
              event.getEventHeaders().get("Other-Leg-Unique-ID"),
        ...
        case "bridge-caller-stop":
          log.info(
              "fifoinfoEventHandler bridge-caller-stop:{},{}",
              addr,
              EslHelper.formatEslEvent(event));
          ...
          if (busy != null) {
            ...
            EslMessage eslMessage =
                inboundClient.sendSyncApiCommand(addr, "uuid_kill", userinfoPojo.getCalluuid());
            log.info(
                "摘机坐席{}置忙{}", userinfoPojo.getUsername(), EslHelper.formatEslMessage(eslMessage));
            if (busy.equals(-1)) {
              userinfoPojo.setWorkStatus(-1);
              msgOperations.convertAndSend(
                  "/topic/status/" + userinfoPojo.getUsername(), userinfoPojo);
        ...
        case "consumer_stop":
          log.info(
              "fifoinfoEventHandler consumer_stop:{},{},{}",
              addr,
              EslHelper.formatEslEvent(event),
              event.getEventHeaders().get("variable_last_sent_callee_id_number"));
          break;

        default:
          log.info(
              "fifoinfoEventHandler 未知:{},{},{}",
              addr,
              EslHelper.formatEslEvent(event),
              event.getEventHeaders().get("FIFO-Action"));
      }
    } catch (Exception ex) {
