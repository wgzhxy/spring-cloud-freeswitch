...
public class ChannelHangupEslEventHandler implements EslEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    log.info(
        "ChannelHangupEslEventHandler handle addr[{}] EslEvent[{}].",
        addr,
    ...
    if (StringUtils.isNotBlank(event.getEventHeaders().get("variable_return_visit"))
      ...
      msgOperations.convertAndSend(
          "/topic/bye/" + event.getEventHeaders().get("Other-Leg-Caller-ID-Number"),
      ...
      if (event.getEventHeaders().get("Hangup-Cause").equals("UNALLOCATED_NUMBER")
          || event.getEventHeaders().get("Hangup-Cause").equals("NO_USER_RESPONSE")) {
        ...
        msgOperations.convertAndSend(
            "/topic/notify/" + userinfoPojo.getUsername(), JsonTool.getFormatJsonString(notify));
      }
      ...
      body.put("callTaskCode", event.getEventHeaders().get("variable_callcenter_task_code"));
      msgOperations.convertAndSend(
          "/topic/bye/" + event.getEventHeaders().get("Caller-Destination-Number"),
          JsonTool.getJsonString(body));
