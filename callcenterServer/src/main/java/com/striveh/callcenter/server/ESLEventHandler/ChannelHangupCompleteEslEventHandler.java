...
public class ChannelHangupCompleteEslEventHandler implements EslEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    ...
    try {
      log.info(
          "ChannelHangupCompleteEslEventHandler handle addr[{}] EslEvent[{}].",
          addr,
      ...
      if (StringUtils.isNotBlank(event.getEventHeaders().get("variable_return_visit"))
        ...
        if (StringTool.isEmpty(
            event.getEventHeaders().get("variable_proto_specific_hangup_cause"))) {
          if (event.getEventHeaders().get("Hangup-Cause").equals("NORMAL_CLEARING")) {
            ...
            if (callLogPojo.getBillsec().equals(0L)) {
              ...
              .getEventHeaders()
              .get("Hangup-Cause")
              .equals("RECOVERY_ON_TIMER_EXPIRE")) {
          ...
          if (event.getEventHeaders().get("Hangup-Cause").equals("NORMAL_CLEARING")) {
            ...
            callLogPojo.setResult("10");
              ...
              || event.getEventHeaders().get("Hangup-Cause").equals("NO_ANSWER")) {
            callLogPojo.setResultDetail("25");
            callLogPojo.setResult("20");
        ...
        try {
          if (callLogPojo.getResult().equals("10")) {
            ...
            if (file.exists()) {
              String storeKey =
                  DateTool.formatDate("yyyyMMdd")
                      + "_"
                      + StringTool.MD5Encode(event.getEventHeaders().get("variable_uuid"))
                      + ".mp3";
              storageService.store(FileUtils.openInputStream(file), 0L, "audio/mp3", storeKey);
              callLogPojo.setRecordFileQiniu(
                  storageService.getStorage().getBucketName() + "|" + storeKey);
              file.delete();
            }
          }
          if (StringUtils.isNotEmpty(callLogPojo.getPriceRule())) {
            String[] priceRule = callLogPojo.getPriceRule().split("\\|");
            callLogPojo.setPriceCal(
                Math.ceil(((double) callLogPojo.getBillsec()) / Double.valueOf(priceRule[0]))
                    * Double.valueOf(priceRule[1]));
          }
        } catch (Exception e) {
        ...
        callLogPojo = this.callLogService.selectUnique(callLogPojo);
        baseRedisDao.unorderedzrem(
            ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode()
                + callLogPojo.getCallTaskCode()
                + "_"
                + callLogPojo.getGwCode(),
            callLogPojo.getDestinationNumber());
        ...
        if (StringTool.isEmpty(
          ...
          if (event.getEventHeaders().get("Hangup-Cause").equals("NORMAL_CLEARING")) {
            ...
            baseRedisDao.unorderedzadd(
                ERedisCacheKey.CALLCENTER_TASK_FAIL_REJECY_COUNT.getCode()
                    + callLogPojo.getCallTaskCode(),
                callLogPojo.getDestinationNumber(),
                0);
          } else if (event.getEventHeaders().get("Hangup-Cause").equals("USER_BUSY")) {
      ...
      if (callLogPojo != null) {
        baseRedisDao.unorderedzrem(
            ...
            callLogPojo.getDestinationNumber());
        baseRedisDao.unorderedzrem(
            ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode() + callLogPojo.getCallTaskCode(),
            callLogPojo.getDestinationNumber());
        ...
        if (StringUtils.isNotEmpty(event.getEventHeaders().get("Other-Leg-Destination-Number"))
          ...
          callLogPojo.setDestinationNumber(
              event
                  .getEventHeaders()
                  .get("Other-Leg-Destination-Number")
                  .substring(
                      event.getEventHeaders().get("Other-Leg-Destination-Number").length() - 11));
        } else {
          callLogPojo.setDestinationNumber(
        ...
        if (callLogPojo != null) {
          ...
          if (StringUtils.isNotEmpty(
              event.getEventHeaders().get("variable_fifo_record_template"))) {
            try {
              callLogPojo.setRecordFile(
                  event.getEventHeaders().get("variable_fifo_record_template"));

              File file = new File(event.getEventHeaders().get("variable_fifo_record_template"));
              //                        String storeKey =
              // event.getEventHeaders().get("variable_fifo_record_template").substring(event.getEventHeaders().get("variable_fifo_record_template").lastIndexOf("/")+1);
              String storeKey =
                  DateTool.formatDate("yyyyMMdd")
                      + "_"
                      + StringTool.MD5Encode(event.getEventHeaders().get("variable_sip_call_id"))
                      + ".mp3";
              storageService.store(FileUtils.openInputStream(file), 0L, "audio/mp3", storeKey);
              callLogPojo.setRecordFileQiniu(
                  storageService.getStorage().getBucketName() + "|" + storeKey);
      ...
      log.error(
          "ChannelHangupCompleteEslEventHandler处理异常{}:{}。。。",
          event.getEventHeaders().get("variable_callcenter_task_code"),
  ...
  private void setEvaluation(CallLogPojo callLogPojo) {
    try {
      Integer evaluation =
          baseRedisDao.get(
              ERedisCacheKey.CALLCENTER_AGENT_SESSION_EVALUATION.getCode()
                  + callLogPojo.getDestinationNumber(),
              Integer.class);
      if (evaluation != null) {
  ...
  private void resultProc(String addr, EslEvent event, CallLogPojo callLogPojo) {
    if (StringTool.isEmpty(event.getEventHeaders().get("variable_proto_specific_hangup_cause"))) {
      if (!StringUtils.isEmpty(event.getEventHeaders().get("variable_task_schedule_type"))
        ...
        callLogPojo.setResult("10");
        baseRedisDao.unorderedzrem(
            ERedisCacheKey.CALLCENTER_TASK_SESSION_COUNT.getCode() + callLogPojo.getCallTaskCode(),
            callLogPojo.getDestinationNumber());
        countSuccess(callLogPojo);
      } else {
        if (event.getEventHeaders().get("Hangup-Cause").equals("NORMAL_CLEARING")) {
          if (StringUtils.isEmpty(event.getEventHeaders().get("Other-Leg-Caller-ID-Number"))) {
            ...
            baseRedisDao.unorderedzadd(
                ERedisCacheKey.CALLCENTER_TASK_MISS_COUNT.getCode() + callLogPojo.getCallTaskCode(),
                callLogPojo.getDestinationNumber(),
                0);
          } else {
            callLogPojo.setResultDetail("50");
    ...
    callLogPojo.setDuration(Long.valueOf(event.getEventHeaders().get("variable_duration")));
    callLogPojo.setBillsec(Long.valueOf(event.getEventHeaders().get("variable_billsec")));
    if (callLogPojo.getHangupCase() == null)
      callLogPojo.setHangupCase(event.getEventHeaders().get("Hangup-Cause"));
    try {

      if (callLogPojo.getResult().equals("10")) {
        Long answeredTime =
            Long.valueOf(event.getEventHeaders().get("Caller-Channel-Answered-Time"));
        Long bridgedTime = Long.valueOf(event.getEventHeaders().get("Caller-Channel-Bridged-Time"));
        if (callLogPojo.getHangupCase() == null)
          callLogPojo.setHangupCase(
              new Timestamp(answeredTime / 1000) + "|" + new Timestamp(bridgedTime / 1000));
  ...
  private void countSuccess(CallLogPojo callLogPojo) {
    baseRedisDao.unorderedzrem(
        ERedisCacheKey.CALLCENTER_TASK_FAIL_COUNT.getCode() + callLogPojo.getCallTaskCode(),
        callLogPojo.getDestinationNumber());
    baseRedisDao.unorderedzadd(
        ERedisCacheKey.CALLCENTER_TASK_SUCCESS_COUNT.getCode() + callLogPojo.getCallTaskCode(),
        callLogPojo.getDestinationNumber(),
        0);
  }

  private void countFail(CallLogPojo callLogPojo) {
    if (!baseRedisDao.sismember(
        ERedisCacheKey.CALLCENTER_TASK_SUCCESS_COUNT.getCode() + callLogPojo.getCallTaskCode(),
        callLogPojo.getDestinationNumber())) {
      baseRedisDao.unorderedzadd(
          ERedisCacheKey.CALLCENTER_TASK_FAIL_COUNT.getCode() + callLogPojo.getCallTaskCode(),
