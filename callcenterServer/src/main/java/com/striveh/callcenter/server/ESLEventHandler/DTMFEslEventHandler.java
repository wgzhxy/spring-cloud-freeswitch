...
public class DTMFEslEventHandler implements EslEventHandler {
  ...
  public void handle(String addr, EslEvent event) {
    ...
    log.info(
        "DTMFEslEventHandler handle addr[{}] EslEvent[{}].", addr, EslHelper.formatEslEvent(event));
    ...
    if (evaluation != null && evaluation.equals(1)) {
      ...
      if (StringUtils.isNumeric(dtmfDigit)) {
        baseRedisDao.saveOrUpdate(
            ERedisCacheKey.CALLCENTER_AGENT_SESSION_EVALUATION.getCode() + telNo, dtmfDigit, 60);
      }

      EslMessage eslMessagePlay =
      ...
      log.info(
          "播放挂断{}结果{}",
          event.getEventHeaders().get("Caller-Unique-ID"),
          EslHelper.formatEslMessage(eslMessagePlay));
    }
  }
}
