...
/**
 * @功能:【callTask 呼叫任务表】Service @项目名:callcenterServer @作者:xxx @日期:2020-04-06 12:13:48 @说明：
 *
 * <pre></pre>
 */
@Service
public class CallTaskService extends BaseService<CallTaskPojo, CallTaskDao>
    implements ICallTaskService {
  ...
  @Autowired private IFreeswitchService freeswitchService;
  @Autowired private IVoiceService voiceService;

  @Autowired private IGatewayServiceApi gatewayServiceApi;
  ...
  public void getCallList(CallTaskPojo callTask) {
    ...
    if (StringTool.isEmpty(callTask.getOriginalCallTaskCode())) {
      ...
      do {
        callList =
            this.callListServiceApi.getListByCallListId(
                Long.valueOf(callTask.getCallTaskCode()), size, lastId);
        if (callList == null || callList.size() == 0) {
          break;
        }
        lastId = callList.get(callList.size() - 1).getId();
        baseRedisDao.saveOrUpdate(
            ERedisCacheKey.CALLCENTER_TASK_CALLLIST_LASTID.getCode() + callTask.getCallTaskCode(),
            lastId);
        callList.forEach(
            e ->
                baseRedisDaoDBCache.leftPush(
                    ERedisCacheKey.CALLCENTER_TASK_CALLLIST.getCode() + callTask.getCallTaskCode(),
                    e));

        callListTotal = callListTotal + callList.size();
        baseRedisDao.saveOrUpdate(
            ERedisCacheKey.CALLCENTER_TASK_TOTAL_COUNT.getCode() + callTask.getCallTaskCode(),
            callListTotal);
      ...
      if (callTask.getOriginalResultDetail().contains("50")
          && callTask.getOriginalResultDetail().contains("55")) {
      ...
      callListSet.forEach(
          e -> {
            if (!telNoSuccessSet.contains(e)) {
              ...
              baseRedisDaoDBCache.leftPush(
                  ERedisCacheKey.CALLCENTER_TASK_CALLLIST_REPEAT.getCode()
                      + callTask.getOriginalCallTaskCode(),
                  callLogPojo);
            }
          });
      baseRedisDao.saveOrUpdate(
          ERedisCacheKey.CALLCENTER_TASK_TOTAL_COUNT.getCode() + callTask.getCallTaskCode(),
          baseRedisDaoDBCache.llen(
              ERedisCacheKey.CALLCENTER_TASK_CALLLIST.getCode() + callTask.getCallTaskCode()));
      try {
  ...
  public void start(CallTaskPojo callTask) {
    baseRedisDao.saveOrUpdate(
        ERedisCacheKey.CALLCENTER_TASK_STATUS.getCode() + callTask.getCallTaskCode(), 2);
    // 拨打类型，1预测式拨打2预览式拨打
  ...
  private void callCustomer(CallTaskPojo callTask) {
    ...
    if (callTask.getScheduleType().equals(1)) {
      ...
      userinfoPojos.forEach(
          e -> {
            if (e.getStatus() == 1 && e.getWorkStatus() == 1) {
              // 加入到呼叫队列坐席列表
              EslMessage eslMessage =
                  inboundClient.sendSyncApiCommand(
                      freeswitchService.getServerAddr(e.getFreeswitchId()),
                      "fifo_member add",
                      callTask.getProjectCode()
                          + " {fifo_record_template=$${recordings_dir}/${strftime(%Y-%m-%d-%H-%M-%S)}."
                          + e.getUsername()
                          + ".mp3,"
                          + "sip_h_X-Call-Task-Code="
                          + callTask.getCallTaskCode()
                          + "}user/"
                          + e.getUsername()
                          + " 1 5 0");
              ...
              logger.info(
                  "{}加入到呼叫队列{}坐席列表结果{}",
                  e.getUsername(),
                  finalCallProjectPojo.getProjectCode(),
    ...
    Integer gwIndex = 0;
    do {
      List<GatewayPojo> callGW =
          JsonTool.getList(
              baseRedisDao.get(
                  ERedisCacheKey.CALLCENTER_TASK_GWS.getCode() + callTask.getCallTaskCode(),
                  String.class),
              GatewayPojo.class);
      // 执行中的呼叫
      Integer runningCount =
          Math.toIntExact(
              baseRedisDao.unorderedZcard(
                  ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode()
                      + callTask.getCallTaskCode()));
      // 可以通话的坐席
      ...
      Double rate =
          baseRedisDao.get(
              ERedisCacheKey.CALLCENTER_TASK_RATE.getCode() + callTask.getCallTaskCode(),
              Double.class);
      Long callListNum =
          baseRedisDaoDBCache.llen(
              ERedisCacheKey.CALLCENTER_TASK_CALLLIST.getCode() + callTask.getCallTaskCode());
      while ((Math.floor(agentCount * rate) <= runningCount) && callListNum > 0) {
        if (2
            != baseRedisDao.get(
                ERedisCacheKey.CALLCENTER_TASK_STATUS.getCode() + callTask.getCallTaskCode(),
                Integer.class)) {
          return;
        }
        try {
          logger.info(
              "任务{}已达最大并发执行数{}等待{}毫秒,agentCount:{},rate:{}",
              callTask.getCallTaskCode(),
              runningCount,
              sleep,
              agentCount,
      ...
      logger.info(
          "任务{}本次执行{}个agentCount:{},rate:{},running:{}",
          callTask.getCallTaskCode(),
          (Math.floor(agentCount * rate) - runningCount),
          agentCount,
          rate,
          runningCount);
      if (callListNum > 0) {
        ...
        for (int i = 0; i < (Math.floor(agentCount * rate) - runningCount); i++) {
          ...
          try {
            Integer pause =
                baseRedisDao.get(
                    ERedisCacheKey.CALLCENTER_TASK_PAUSE.getCode() + callTask.getCallTaskCode(),
                    Integer.class);
            if (pause != null) {
          ...
          CallListPojo callListPojo =
              baseRedisDaoDBCache.rightPop(
                  ERedisCacheKey.CALLCENTER_TASK_CALLLIST.getCode() + callTask.getCallTaskCode(),
                  CallListPojo.class);
          ...
          try {
            ...
            String phonePrefix = "";
            if (StringTool.isPhoneNumber(telNo)
                && StringUtils.isNotEmpty(gatewayPojo.getMobilePhonePrefix())) {
              callTelNo = gatewayPojo.getMobilePhonePrefix() + telNo;
              phonePrefix = gatewayPojo.getMobilePhonePrefix();
            } else if (!StringTool.isPhoneNumber(telNo)
                && StringUtils.isNotEmpty(gatewayPojo.getFixedPhonePrefix())) {
            ...
            if (gatewayPojo.getRegisterType() == 1) {
              ...
              EslMessage eslMessage =
                  inboundClient.sendSyncApiCommand(
                      freeswitchService.getServerAddr(callProjectPojo.getFreeswitchId()),
                      "sofia_contact",
                      gatewayPojo.getUsername());
              logger.info(
                  "网关用户 sofia_contact {},结果{}",
                  gatewayPojo.getUsername(),
                  EslHelper.formatEslMessage(eslMessage));
            ...
            String callStr =
                "{originate_timeout="
                    + originateTimeout
                    + ",ignore_early_media=true,sip_sticky_contact=true,fifo_music=local_stream://moh"
                    + (StringUtils.isNotEmpty(gatewayPojo.getOriginationCallId())
                        ? ",origination_caller_id_number=" + gatewayPojo.getOriginationCallId()
                        : "")
                    + (StringUtils.isNotEmpty(gatewayPojo.getCodec())
                        ? ",absolute_codec_string=" + gatewayPojo.getCodec()
                        : "")
                    + (callTask.getScheduleType().equals(5) ? ",task_schedule_type=playback" : "")
                    + ",execute_on_answer='record_session "
                    + recordingsPath
                    + DateTool.formatDate("yyyyMMddHHmmss")
                    + telNo
                    + ".mp3'"
                    + ",callcenter_project_code="
                    + callTask.getProjectCode()
                    + ",callcenter_task_code="
                    + callTask.getCallTaskCode()
                    + ",phonePrefix="
                    + phonePrefix
                    + "}"
            ...
            // 记录执行中的呼叫
            baseRedisDao.unorderedzadd(
                ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode() + callTask.getCallTaskCode(),
                telNo,
                0);
            baseRedisDao.unorderedzadd(
                ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode()
            ...
            baseRedisDao.unorderedzadd(
                ERedisCacheKey.CALLCENTER_TASK_FAIL_COUNT.getCode() + callTask.getCallTaskCode(),
                telNo,
                0);
            // 记录calllog
            CallLogPojo callLogPojo = new CallLogPojo();
   ...
   * @param callApp
   */
  private void callTaskNoAgentByGatewayConcurrency(
      CallProjectPojo callProjectPojo, CallTaskPojo callTask, String callApp) {
    Integer gwIndex = 0;
    do {
      List<GatewayPojo> callGW =
          JsonTool.getList(
            ...
            String callStr =
                "{originate_timeout="
                    ...
                    + (StringUtils.isNotEmpty(gatewayPojo.getCodec())
                        ? ",absolute_codec_string=" + gatewayPojo.getCodec()
                        : "")
                    + (callTask.getScheduleType().equals(4)
                        ? ",execute_on_answer='record_session "
                            + recordingsPath
                            + DateTool.formatDate("yyyyMMddHHmmss")
                            + telNo
  ...
  private void clearCallTask(CallTaskPojo callTask, CallProjectPojo callProjectPojo) {
    if (callTask.getExpiredTime().getTime() > System.currentTimeMillis()) {
      ...
      while (0 != runningCount || sessionCount > 0) {
        try {
          logger.info(
              "任务{}还有{}个在执行,或者还有{}个在通话。。。", callTask.getCallTaskCode(), runningCount, sessionCount);
          Thread.sleep(5000);
        } catch (InterruptedException e) {
          logger.info(e);
        }
    ...
    if (callTask.getScheduleType().equals(1)) {
      this.userInfoServiceApi
          .getListByProjectId(callProjectPojo.getId())
          .forEach(
              e -> {
                String param =
                    callTask.getProjectCode()
    ...
    if (null
      ...
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_CALLLIST_LASTID.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_SUCCESS_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_FAIL_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_FAIL_REJECY_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_MISS_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_TOTAL_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode() + callTask.getCallTaskCode());
      baseRedisDao.delete(
          ERedisCacheKey.CALLCENTER_TASK_SESSION_COUNT.getCode() + callTask.getCallTaskCode());
  ...
  public void pause(CallTaskPojo callTask) {
    baseRedisDao.saveOrUpdate(
        ERedisCacheKey.CALLCENTER_TASK_STATUS.getCode() + callTask.getCallTaskCode(), 3);
  ...
  public void notifyCount(CallTaskPojo e, String uncallListSyncStatus) {
    ...
    Integer totalCount =
        baseRedisDao.get(
            ERedisCacheKey.CALLCENTER_TASK_TOTAL_COUNT.getCode() + e.getCallTaskCode(),
            Integer.class);
    if (totalCount == null) {
      return;
    }
    ...
    Long failCount =
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_FAIL_COUNT.getCode() + e.getCallTaskCode());
    params.put("failCnt", String.valueOf(failCount));
    Long rejectCnt =
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_FAIL_REJECY_COUNT.getCode() + e.getCallTaskCode());
    params.put("rejectCnt", String.valueOf(rejectCnt));
    Long missCount =
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_MISS_COUNT.getCode() + e.getCallTaskCode());
    params.put("missCnt", String.valueOf(missCount));
    Long successCount =
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_SUCCESS_COUNT.getCode() + e.getCallTaskCode());
    params.put("successCnt", String.valueOf(successCount));
    params.put("uncallListSyncStatus", uncallListSyncStatus);
    Long runningCount =
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_RUNNING_COUNT.getCode() + e.getCallTaskCode());
    ...
    if (e.getExpiredTime().getTime() < System.currentTimeMillis() && e.getStatus() < 4) {
      ...
      if (e.getStatus().equals(1)) {
        if ((e.getUpdateTime() == null
            && (DateTool.getTimestamp().getTime() - e.getAddTime().getTime()) > 5 * 60 * 1000)) {
          return;
        } else if ((e.getUpdateTime() != null
            && (DateTool.getTimestamp().getTime() - e.getUpdateTime().getTime()) > 5 * 60 * 1000)) {
          return;
        }
      }
  ...
  public void callTimeout(CallTaskPojo e) {
    ...
    this.callLogService
        .selectListByCalling(callLogPojo)
        .forEach(
            callLogPojo1 -> {
              EslMessage eslMessage =
                  inboundClient.sendSyncApiCommand(
                      freeswitchService.getServerAddr(finalCallProjectPojo.getFreeswitchId()),
                      "uuid_kill",
                      callLogPojo1.getCallUUID());
              logger.info(
                  "任务{}号码{}呼叫超时{},结果{}",
                  callLogPojo1.getCallTaskCode(),
                  callLogPojo1.getDestinationNumber(),
                  callLogPojo1.getCallUUID(),
                  EslHelper.formatEslMessage(eslMessage));
            });
  }
  ...
  public void reloadGateway(GatewayPojo gatewayPojo) {
    inboundClient
        ...
        .forEach(
            e -> {
              EslMessage eslMessageDel =
                  inboundClient.sendSyncApiCommand(
                      e.addr(), "sofia profile external killgw", gatewayPojo.getGwCode());
              EslMessage eslMessage =
                  inboundClient.sendSyncApiCommand(e.addr(), "sofia profile external rescan", null);
              logger.info(
                  "FS{}重新加载网关{}结果{}",
                  e.addr(),
                  gatewayPojo.getGwName(),
                  EslHelper.formatEslMessage(eslMessage));
            });
  }
  ...
  public void previewCall(String jsonStr) {
    ...
    if (callTask.getStatus().equals(4)) {
      ...
      msgOperations.convertAndSend(
          "/topic/notify/" + callTask.getProjectCode() + "/" + username,
          JsonTool.getFormatJsonString(notify));
      return;
    ...
    if (gatewayPojo.getRegisterType() == 1) {
      ...
      EslMessage eslMessage =
          inboundClient.sendSyncApiCommand(
              freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()),
              "sofia_contact",
              gatewayPojo.getUsername());
      logger.info(
          "网关用户 sofia_contact {},结果{}",
    ...
    EslMessage eslMessage =
        inboundClient.sendSyncApiCommand(
            freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()), "create_uuid", null);
    ...
    String callStr =
        "{preview_call=true,callcenter_project_code="
            + callTask.getProjectCode()
            + ",callcenter_task_code="
            + callTask.getCallTaskCode()
            + ",phonePrefix="
            ...
            + "}user/"
            + username
            + " &bridge({sip_sticky_contact=true,preview_call=true,origination_uuid="
            + eslMessage.getBodyLines().get(0)
            + (StringUtils.isNotEmpty(gatewayPojo.getOriginationCallId())
                ? ",origination_caller_id_number=" + gatewayPojo.getOriginationCallId()
                : "")
            + (StringUtils.isNotEmpty(gatewayPojo.getCodec())
    ...
    if (newCallLogPojo != null
        && (StringTool.isEmpty(newCallLogPojo.getHangupCase())
            || newCallLogPojo.getHangupCase().equals("NO_CALL_DISTRIBUTION"))) {
    ...
    logger.info(
        ...
        callStr,
        inboundClient.sendAsyncApiCommand(
            freeswitchService.getServerAddr(userinfoPojo.getFreeswitchId()), "originate", callStr));
  ...
  public List<CallListPojo> getCallList(CallTaskPojo callTaskPojo, Integer count) {
    ...
    for (int i = 0; i < listSize; i++) {
      CallListPojo callListPojo =
          baseRedisDaoDBCache.rightPop(
              ERedisCacheKey.CALLCENTER_TASK_CALLLIST.getCode() + callTaskPojo.getCallTaskCode(),
              CallListPojo.class);
      if (callListPojo != null) {
  ...
  public void agentCall(Map<String, String> map, String username, String telNo) {
    ...
    try {
      ...
      Integer callCount =
          baseRedisDao.get(
              ERedisCacheKey.CALLCENTER_TEST_CALL_DAY_COUNT.getCode() + username, Integer.class);
      if (callLogPojo == null || StringUtils.isEmpty(callLogPojo.getCallTaskCode())) {
        if (callCount != null && callCount >= 20) {
          ...
          msgOperations.convertAndSend(
              "/topic/notify/" + map.get("projectCode") + "/" + username,
              JsonTool.getFormatJsonString(notify));
          return;
        }
      }
      ...
      String callStr =
          "{return_visit=true"
              + (callLogPojo == null || callLogPojo.getCallTaskCode() == null
                  ? ",execute_on_answer='sched_hangup +1800'"
                  : "")
              + (callLogPojo != null && callLogPojo.getCallTaskCode() != null
                  ? ",callcenter_project_code="
                      + callLogPojo.getProjectCode()
                      + ",callcenter_task_code="
                      + callLogPojo.getCallTaskCode()
                  : "")
              + "}user/"
              + username
              + " &bridge({sip_sticky_contact=true,return_visit=true,origination_uuid="
              + eslMessage.getBodyLines().get(0)
              + (StringUtils.isNotEmpty(gatewayPojo.getOriginationCallId())
                  ? ",origination_caller_id_number=" + gatewayPojo.getOriginationCallId()
                  : "")
      ...
      if (callLogPojo != null && callLogPojo.getCallTaskCode() != null) {
        ...
        baseRedisDao.saveOrUpdate(
            ERedisCacheKey.CALLCENTER_TEST_CALL_DAY_COUNT.getCode() + username,
            callCount + 1,
            Long.valueOf(
                    (86400 - DateUtils.getFragmentInSeconds(Calendar.getInstance(), Calendar.DATE)))
                .intValue());
      }
      newCallLogPojo.setDestinationNumber(telNo);
      newCallLogPojo.setGwCode(gatewayPojo.getGwCode());
            ...
            if (e.getStatus() == 1 && e.getWorkStatus() == 1 && e.getSessionStatus().equals(0)) {
              if (callTaskPojo.getScheduleType().equals(4)) {
                ...
                if (eslMessage.getBodyLines().get(0).contains("true")) {
                  ...
                  //                        msgOperations.convertAndSend("/topic/status/" +
                  // e.getUsername(), e);
  ...
  public Integer getAgentSessionCnt(CallTaskPojo callTaskPojo) {
    return Math.toIntExact(
        baseRedisDao.unorderedZcard(
            ERedisCacheKey.CALLCENTER_TASK_SESSION_COUNT.getCode()
                + callTaskPojo.getCallTaskCode()));
  }

  public boolean isAvailableGw(GatewayPojo gatewayPojo) {
    Map<String, Integer> gw = new HashMap<>();
    callTaskPojos.forEach(
        e -> {
