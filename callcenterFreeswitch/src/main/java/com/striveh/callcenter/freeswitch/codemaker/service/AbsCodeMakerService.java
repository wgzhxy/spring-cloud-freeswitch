...
public abstract class AbsCodeMakerService {
   ...
   * @throws Exception
   */
  public void generateCode(CodeMakerCfg cfg, SqlSessionTemplate sqlSessionTemplate)
      throws Exception {
    /** 取得列信息，设置类名及java数据类型 */
    ...
    String templateBasePath =
        CodeMakerCfg.class.getClass().getResource(cfg.getTemplateBasePath()).getFile();
   ...
   * @retrun 项目根文件夹
   */
  private File getDataMap(
      Map<String, Object> dataMap, CodeMakerCfg cfg, List<ColumnInfo> columnList) {
    ...
    if (cfg.getProjectName().contains(":")
        || cfg.getProjectName().contains("/")
        || cfg.getProjectName().contains("\\\\")) {
      ...
      String project =
          currentProjectPath.substring(0, currentProjectPath.lastIndexOf("/") + 1)
              + cfg.getProjectName();
