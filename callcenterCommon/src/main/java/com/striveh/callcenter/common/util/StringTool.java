...
public class StringTool {
  ...
  public static java.sql.Date getBirthdayByIdCardNo(String idCardNo, boolean isCheckNo) {
    ...
    try {
      ...
      if (isRight && idCardNo.length() == 18) {
        result =
            new java.sql.Date(DateTool.parseDate("yyyyMMdd", idCardNo.substring(6, 14)).getTime());
      }
    } catch (Exception ex) {
   ...
   *
   * <p>comparison that works for version strings. e.g. "1.10".compareTo("1.6").
   *
   * @note It does not work if "1.10" is supposed to be equal to "1.10.0".
   * @param str1 a string of ordinal numbers separated by decimal points.
   * @param str2 a string of ordinal numbers separated by decimal points.
   * @return The result is a negative integer if str1 is _numerically_ less than str2.
   *     <p>The result is a positive integer if str1 is _numerically_ greater than str2.
   *     <p>The result is zero if the strings are _numerically_ equal.
   */
