...
@Service
public class SysAuthInfoService extends BaseCacheService<SysAuthInfoPojo, SysAuthInfoDao>
    implements ISysAuthInfoService {
  ...
  public void init() {
    ...
    if (lastVer == null) {
      ...
      baseRedisDao.saveOrUpdate(
          ERedisCacheKey.KEY_SYS_AUTHINFO_NEW_VER.getCode(), lastVer); // 更新新版本为当前
    }
  ...
  public void cleanCache(SysAuthInfoPojo po) {
    ...
    baseRedisDao.saveOrUpdate(
        ERedisCacheKey.KEY_SYS_AUTHINFO_NEW_VER.getCode(), newVer); // 更新新版本为当前
    init();
  }
  ...
  public OptResult chenkSign(String account, String timestamp, String sign, Integer type) {
    ...
    if (Math.abs(s) > authValidTime * 60000) {
      return EResCodeCommon.svceErrSysAuthInfoTimeOut.getOptResult(
          logger, "authInfo签名超过" + authValidTime + "分钟,实际：" + (s / 1000) + "秒");
    }
